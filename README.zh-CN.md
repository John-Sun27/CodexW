...
```text
CodexWLauncher.exe           原生 Windows 启动器，包含在 Release 下载包中。
Start-CodexW.cmd             备用根目录启动器。
windows/Start-CodexW.cmd     隐藏命令行窗口的启动器。
windows/CodexW.ps1           WPF 主程序。
Resources/CodexW-icon.png    托盘和标题图标。
