...
- Codex 用量桌面常显面板。
- 完整仪表盘模式和紧凑简洁模式。简洁模式刷新时只请求本机账户实时额度，不扫描会话日志；切回完整模式后会自动进行一次统计全量刷新。
- 额度圆环会适配当前 Codex 或 ChatGPT 桌面应用实际提供的窗口；仅提供每周额度时只显示一个每周圆环。
...
- 本机存在 Codex 或 ChatGPT 桌面数据目录；CodexW 会优先检查 `%USERPROFILE%\.chatgpt`，并兼容 `%USERPROFILE%\.codex`。
...
CodexW 通过本机 Codex 账户接口读取当前套餐和剩余额度。完整模式刷新时还会读取本机 Codex JSONL session 日志以生成统计和任务；简洁模式会跳过该扫描。CodexW 不会上传你的本地用量数据。
